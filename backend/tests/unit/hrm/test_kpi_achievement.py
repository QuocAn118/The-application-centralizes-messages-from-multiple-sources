...
class TestTinhPhanTramKpi:
    def test_cao_la_tot_thuc_dat_tren_muc_tieu(self) -> None:
        ...
        pt = tinh_phan_tram_kpi(KpiMetricType.CONVERSATIONS_CLOSED, Decimal(10), Decimal(8))
        assert pt == Decimal("80.0")

    def test_cao_la_tot_vuot_muc_tieu_tren_100(self) -> None:
        pt = tinh_phan_tram_kpi(KpiMetricType.CONVERSATIONS_CLOSED, Decimal(10), Decimal(12))
    ...
    def test_thap_la_tot_nhanh_hon_tren_100(self) -> None:
        ...
        pt = tinh_phan_tram_kpi(KpiMetricType.AVG_RESPONSE_MINUTES, Decimal(10), Decimal(5))
        assert pt == Decimal("200.0")

    def test_thuc_dat_none_tra_none(self) -> None:
        assert tinh_phan_tram_kpi(KpiMetricType.CONVERSATIONS_CLOSED, Decimal(10), None) is None

    def test_mau_so_khong_tra_none(self) -> None:
        # cao-là-tốt với target 0 → không xác định.
        assert (
            tinh_phan_tram_kpi(KpiMetricType.CONVERSATIONS_CLOSED, Decimal(0), Decimal(5)) is None
        )
        # thấp-là-tốt với actual 0 → không xác định.
        assert (
            tinh_phan_tram_kpi(KpiMetricType.AVG_RESPONSE_MINUTES, Decimal(10), Decimal(0)) is None
        )
