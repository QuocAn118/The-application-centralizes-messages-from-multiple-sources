...
**Nợ GĐ4 — ĐÃ TRẢ (2026-08-04):** `assigned_count` từng = 0 (thiếu `assignment_log`). Đã thêm bảng `assignment_log` (#3, ghi khi `AssignResult.ASSIGNED`, cùng giao dịch với việc gán) và backfill đếm từ đó theo phòng HIỆN TẠI (event-time). KHÔNG thêm `post_assign_agent_hooks` — chốt: log là nguồn sự thật, "hôm nay" cập nhật qua `RebuildDailyRollup` định kỳ. Xem `docs/superpowers/adr/2026-08-04-assignment-log-va-rollup-dong-bo.md`.
...
- **Nợ (cập nhật 2026-08-04):** (a) ~~`assignment_log` chưa có~~ → ĐÃ LÀM: bảng `assignment_log`, `assigned_count` đếm đủ mọi lần gán (ADR 2026-08-04); (b) rollup chạy đồng bộ trong request — GIỮ NGUYÊN có chủ đích, ghi ADR + tiêu chí xét lại (2026-08-04); (c) "hôm nay" đọc thẳng rollup, cần `RebuildDailyRollup` định kỳ (nợ độ trễ — chấp nhận cho báo cáo); (d) không CSV; (e) ~~KPI routing metric nợ mở~~ → ĐÃ LÀM (2026-08-04): `workforce_rows` đổ `kpi_percent` = % `CONVERSATIONS_CLOSED` cho tháng của `to_date`, `period` = `YYYY-MM`; dùng chung service `tinh_phan_tram_kpi` với #3/#4.
