...
- **NỢ KPI — ĐÃ TRẢ (2026-08-04):** chốt nghiệp vụ: chỉ số định tuyến = `CONVERSATIONS_CLOSED`, kỳ = **tháng hiện tại** (theo `app_timezone`), thiếu target → `None` (giữ selector trung tính). `HrmIdentityAgentPool` giờ ghép target (#4 KpiTarget) với thực đạt (#4 nguồn hiệu suất Inbox) qua domain service dùng chung `tinh_phan_tram_kpi` (tách khỏi `GetKpiProgress`). Đồng thời `last_assigned_at` chuyển từ proxy `max(updated_at)` sang `max(assigned_at)` của `assignment_log` (chính xác). Xem ADR `2026-08-04-assignment-log-va-rollup-dong-bo.md` (mục KPI bổ sung).
